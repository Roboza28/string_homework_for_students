...
def recover_article() -> str:
    ...
    split_wrong_article = wrong_article.split(SPLIT_SYMBOL)
    for i, val in enumerate(split_wrong_article):
        split_wrong_article[i] = val[int(len(val) / 2) - 1::-1].replace("WOOF-WOOF", "CAT").capitalize()
    return '.\n'.join(split_wrong_article)
